...
public class Lawn {
  ...
  public Lawn(int maxX, int maxY) {
    ...
    this.coordinates =
        IntStream.rangeClosed(0, maxY)
            .boxed()
            .flatMap(x -> IntStream.rangeClosed(0, maxX).mapToObj(y -> new Coordinate(x, y)))
  ...
  public void block(Coordinate coordinate) {
    changeStateCoordinate(coordinate, false);
  }

  public void free(Coordinate coordinate) {
    changeStateCoordinate(coordinate, true);
  }

  private void changeStateCoordinate(Coordinate coordinate, boolean b) {
    if (coordinates.containsKey(coordinate)) coordinates.put(coordinate, b);
    else throw new InvalidCoordinate("Invalid coordiante : " + coordinate);
  }
}
